...
export const Committee = async ({ block }: CommitteeProps) => {
  ...
  const { officialRoles, defaultImage } = block

  if (!officialRoles || !Array.isArray(officialRoles)) {
    return null
  }

  return (
    <div className="space-y-8">
      {officialRoles.map((officialRole) => {
        if (typeof officialRole !== 'object') {
          return null
        }

        return (
          <div key={officialRole.id} className="space-y-8">
            {/* Role title */}
              ...
              {officialRole.officials?.map((official) => {
                ...
                return (
                  <div key={official.id} className="flex flex-col overflow-hidden p-4 text-center">
                    ...
                    {/* Telegram nickname */}
                    {official.telegramNick && (
                      <a
                        href={`https://t.me/${official.telegramNick.replace('@', '')}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-fk-gray-light block text-sm hover:underline"
                      >
                        {`@${official.telegramNick.replace('@', '')}`}
                      </a>
                    )}
                  </div>
                )
